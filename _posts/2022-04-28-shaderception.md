...
For these reasons, the compiler is written as a single 1300 line C# script, containing a single class, and which uses arrays as the sole data structure, primarily arrays of `object` or `string`. Data structures such as hashtables are implemented via linear search through simple arrays storing keys and values in an alternating fashion, since real hashtables simply weren't worth the overhead in extern calls. An amazing code style, I know.
