...
The image is scaled such that the bottom left corner corresponds to X-axis partial derivatives of (-1, -1), and the top right corner corresponds to (1, 1). The color at each pixel indicates which mipmap level is being sampled, when `Texture2D.SampleGrad()` is passed partial derivatives equal to the pixel coordinates.
