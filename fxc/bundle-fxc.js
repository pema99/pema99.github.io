...
function __wbg_get_imports() {
  ...
  imports.wbg.__wbg_drawImage_dfdde1887a169a24 = function() {
...
async function main() {
  ...
  function setOutput(text, isError = false) {
    ...
    } else if (text.trimStart().startsWith("<pre>")) {
      const inner = text.replace(/\r\n/g, "\n").replace(/^<pre><body[^>]*>/, "").replace(/<\/pre>\s*$/, "");
      outputEl.innerHTML = inner;
      outputEl.className = "";
    } else {
      outputEl.innerHTML = highlightAsm(text);
      outputEl.className = "";
    }
  }
